...
import { ComponentItem, Field, Attribute, AttributesProps} from "../types";
...
export default function Attributes({
  ...
  const removeAttribute = (name: string) => {
    updateComponent({
      attributes: component.attributes?.filter((attr: Attribute) => attr.name !== name) || [],
    });
  };
  ...
  return (
    <Accordion sx={{ mt: 2 }}>
      ...
      <AccordionDetails>
        <Stack spacing={1}>
          {(component.attributes || []).map((attr: Attribute) => (
            <Stack
              key={attr.name}
              direction="row"
          ...
          <Stack direction="row" spacing={1} sx={{ mt: 1 }}>
            ...
            <FormControl size="small" sx={{ minWidth: 80 }}>
              <Select
                value={newAttrType}
                onChange={(e) => setNewAttrType(e.target.value as Field["type"])}
              >
            ...
            <Button size="small" variant="outlined" onClick={addAttribute}>
              {translate("add")}
            </Button>
          </Stack>
        </Stack>
